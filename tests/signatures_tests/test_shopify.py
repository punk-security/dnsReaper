...
def test_potential_success_match_ipv4():
    ...
    assert shopify.test.potential(domain) == True
...
def test_potential_failure():
    domain = Domain("mock.local", fetch_standard_records=False)
    assert shopify.test.potential(domain) == False
...
def test_check_success():
    ...
    assert shopify.test.check(domain) == True


def test_check_failure():
    domain = Domain("mock.local", fetch_standard_records=False)
    mocks.mock_web_response_with_static_value(domain, "Welcome to my shop!")
    assert shopify.test.check(domain) == False
