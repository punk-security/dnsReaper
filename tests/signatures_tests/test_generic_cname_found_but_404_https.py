...
from signatures import _generic_cname_found_but_404_https
...
async def test_potential_success_with_a_cname():
    ...
    domain.CNAME = ["cname"]
    assert _generic_cname_found_but_404_https.test.potential(domain) == True


@pytest.mark.asyncio
async def test_potential_failure():
    domain = Domain("mock.local", fetch_standard_records=False)
    assert _generic_cname_found_but_404_https.test.potential(domain) == False
...
async def test_check_success():
    ...
    assert await _generic_cname_found_but_404_https.test.check(domain) == True
...
async def test_check_failure():
    ...
    domain = Domain("mock.local", fetch_standard_records=False)
    domain.fetch_web = mock_fetch_web
    assert await _generic_cname_found_but_404_https.test.check(domain) == False
